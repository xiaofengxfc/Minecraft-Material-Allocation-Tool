...
(function () {
    ...
    // 语义父类映射：将变体少的材料归入更宽泛的类别
    const PARENT_CATEGORIES = {
        '红石元件': ['压力板', '按钮', '红石火把', '红石粉', '红石块', '红石灯', '比较器', '中继器',
                     '侦测器', '标靶', '避雷针', '绊线钩', '阳光探测器', '滴石'],
        '围栏类':   ['栅栏', '栅栏门', '栏杆', '铁栏杆', '墙'],
        '砖石类':   ['石砖', '砖块', '红砖块', '下界砖', '下界砖块', '红砖', '砖', '圆石', '石头',
                     '黑石', '深板岩', '凝灰岩', '花岗岩', '闪长岩', '安山岩',
                     '砂岩', '红砂岩', '石英', '玄武岩', '黑曜石', '方解石', '滴水石块', '滴水石锥'],
        '功能方块': ['熔炉', '高炉', '烟熏炉', '酿造台', '制图台', '制箭台', '锻造台', '织布机',
                     '切石机', '讲台', '附魔台', '铁砧', '堆肥桶', '炼药锅', '砂轮', '书架'],
        '容器类':   ['箱子', '木桶', '陷阱箱', '潜影盒', '漏斗'],
        '矿车铁轨': ['漏斗矿车', '箱子矿车', '运输矿车', 'TNT矿车', '铁轨', '探测铁轨', '充能铁轨', '激活铁轨'],
        '光源类':   ['火把', '灯笼', '灵魂火把', '灵魂灯笼', '海晶灯', '蛙明灯', '荧石', '南瓜灯', '蜡烛'],
        '玻璃类':   ['玻璃', '染色玻璃', '玻璃板', '染色玻璃板'],
        '植物类':   ['树苗', '草', '蕨', '花丛', '花簇', '藤蔓', '地衣', '垂根', '树根', '竹笋',
                     '蘑菇', '植株', '茎', '下界疣', '菌岩', '菌丝', '菌光体', '竹笋',
                     '紫颂花', '紫颂植株', '下界疣块', '蘑菇方块'],
        '珊瑚类':   ['珊瑚', '珊瑚块', '珊瑚扇'],
        '锁链格栅': ['锁链', '铁锁链', '格栅'],
        '特殊方块': ['信标', '唱片机', '重生锚', '磁石', '潮涌核心', '龙蛋', '末地烛', '饰纹陶罐',
                     '传送门框架', '试炼刷怪笼', '刷怪笼', '宝库', '重锤核心', '合成器', '海绵',
                     '蛋糕', '蜂巢', '蜂箱', '蜘蛛网', '脚手架', '梯子'],
        '粘性方块': ['粘液块', '蜂蜜块', '蜜脾块', '干草块', '骨块', '干海带块'],
        '告示装饰': ['告示牌', '悬挂告示牌', '营火', '灵魂营火', '画', '物品展示框', '盔甲架',
                     '花盆', '盆栽', '地毯', '旗帜', '蜡烛', '床'],
        '矿石原矿': ['矿石', '原矿', '粗铁', '粗金', '粗铜'],
        '紫水晶类': ['紫水晶母岩', '紫水晶簇', '紫水晶块', '紫水晶', '紫晶芽'],
        '树脂泥砖': ['树脂砖', '树脂块', '树脂簇', '泥砖', '泥坯', '泥巴'],
    };
    ...
    function assignGroups(mats) {
        ...
        // 自动合并小组：≤2 种材料的组查找父类合并
        const mergeTarget = new Map(); // baseType → parentCategory
        for (const [baseType, indices] of groupMap) {
            if (indices.length <= 2) {
                for (const [parent, children] of Object.entries(PARENT_CATEGORIES)) {
                    if (children.includes(baseType)) {
                        mergeTarget.set(baseType, parent);
                        break;
                    }
                }
            }
        ...
        // 执行合并
        for (const [baseType, parent] of mergeTarget) {
            const indices = groupMap.get(baseType);
            if (!groupMap.has(parent)) groupMap.set(parent, []);
            groupMap.get(parent).push(...indices);
            groupMap.delete(baseType);
        }
