...
(function () {
    ...
    // ==================== 材料分组 ====================
    /**
     * Minecraft 材料常见的后缀基类（基础材料类型词尾）
     * 例如 "石砖台阶" 后缀匹配 "台阶" → 提取为 "台阶"
     *       "橡木楼梯" 后缀匹配 "楼梯" → 提取为 "楼梯"
     * 两者归入同一材料组
     * 按长度降序排列，优先匹配更长的后缀（如"染色玻璃"优先于"玻璃"）
     */
    ...
    const MATERIAL_PREFIXES = [
        ...
        '苍白橡木', '红树木', '樱花木', '竹',
        '深板岩', '绯红', '诡异',
        '橡木', '白桦', '云杉', '丛林', '金合欢', '深色', '苍白',
        '红树', '樱花',
        // 石质变种（打磨/装饰前缀 — 后缀匹配优先，这里作为兜底）
        '錾制深板岩', '磨制黑石', '磨制深板岩',
        '裂纹深板岩', '裂纹下界砖',
     ...
     * 从中文材料名称中提取基础类型
     * 两阶段匹配：
     *   1. 后缀匹配（优先）：识别材料名称结尾的基类词
     *      例如 "石砖台阶" → "台阶"，"橡木楼梯" → "楼梯"
     *   2. 前缀剥离（兜底）：剥离颜色/材质前缀
     *      例如 "白色染色玻璃" → "染色玻璃"，"橡木木板" → "木板"
     * @param {string} cnName
     * @returns {string}
     */
    function extractBaseType(cnName) {
        ...
        // 第一阶段：后缀匹配（按长度降序，优先匹配更长后缀）
        const sortedSuffixes = [...MATERIAL_SUFFIXES].sort((a, b) => b.length - a.length);
        for (const suffix of sortedSuffixes) {
            if (cnName.endsWith(suffix) && cnName.length > suffix.length) {
                return suffix;
            }
        }

        // 第二阶段：前缀剥离（按长度降序，优先匹配更长的前缀）
